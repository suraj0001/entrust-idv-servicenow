...
function _idvEnableSave() {
    ...
    if (wrap) { wrap.title = ''; wrap.style.cursor = 'auto' }
}

function _idvDisableSave() {
    _el('btn_save').disabled = true
    var wrap = _el('btn_save_wrap')
    if (wrap) { wrap.title = 'Run a successful Test Connection to enable Save'; wrap.style.cursor = 'not-allowed' }
}
...
_el('btn_test').addEventListener('click', function () {
    ...
    if (!region) { _idvShowStatus('error', 'Please select a region.'); return }

    if (!clientId || !clientSecret) {
        _idvShowStatus('error', _idvHasStoredCredentials
    ...
    if (lenError) { _idvShowStatus('error', lenError); return }

    btn.disabled = true
    btn.textContent = 'Testing…'
    _idvShowStatus('', 'Connecting to Entrust IDV…')

    _ajax('testConnection', {
...
_el('btn_save').addEventListener('click', function () {
    ...
    if (!region || !baseUrl || !tokenUrl) { _idvShowStatus('error', 'Please select a region.'); return }
    ...
    _ajax('getAliasInfo', {}, function (info) {
        ...
        ccGa.addParam('sysparm_formData', JSON.stringify({
