...
export function createApplicant(
  ...
  if (statusCode !== 201) {
    throw new Error(
      "Entrust applicant creation failed. HTTP status: " + statusCode,
    );
  }
...
export function createWorkflowRun(
  ...
  if (statusCode !== 201) {
    throw new Error(
      "Entrust workflow run creation failed. HTTP status: " + statusCode,
    );
  }
...
function createRequest(
  ...
  request.setEndpoint(
    connection.baseUrl.replace(/\/$/, "") + "/" + API_VERSION + path,
  );
