// sn_ws is a ServiceNow server-side global, not in @servicenow/glide stubs
declare const sn_ws: { RESTMessageV2: new () => any }
...
export function createApplicant(
   ...
   if (statusCode !== 201) {
       throw new Error(
           'Entrust applicant creation failed. HTTP status: ' +
           statusCode
       )
   }
   ...
   if (!body.id) {
       throw new Error(
           'Entrust applicant response did not contain an applicant ID.'
       )
   }
   return {
       applicantId: body.id,
...
export function createWorkflowRun(
   ...
   if (input.expiresAt || input.redirectUrl) {
       ...
       if (input.redirectUrl) {
           payload.link.completed_redirect_url =
               input.redirectUrl
           payload.link.expired_redirect_url =
               input.redirectUrl
       }
   }
   ...
   if (statusCode !== 201) {
       throw new Error(
           'Entrust workflow run creation failed. HTTP status: ' +
           statusCode
       )
   }
   const body = JSON.parse(response.getBody())
   if (!body.id || !body.link || !body.link.url) {
       throw new Error(
           'Entrust workflow run response is incomplete.'
       )
   }
   return {
...
function createRequest(connection: EntrustConnection, path: string): any {
    const request = new sn_ws.RESTMessageV2()
    request.setEndpoint(connection.baseUrl.replace(/\/$/, '') + path)
    ...
    request.setRequestorProfile(
        connection.requestorContext,
